...
import "../../../AddResort/image/ImageUpload.css";

import Modal from "../../../../common/Modal";

import ImageCrop from "../../../AddResort/image/ImageCrop";
...
import roomImagesService from "../../../../../services/resort/room/roomImages.service";
...
interface Props {
  ...
  imageUploadHandler: (images: roomImage[])=> void;
  imageDeleteHandler: (id: number) => void;
}
const RoomImageDetails = (props: Props) => {
  const { roomId, roomImages, imageDeleteHandler, imageUploadHandler } = props;
  const [openImageModal, setOpenImageModal] = React.useState(false);
  const [uploadedImages, setUploadedImages] = React.useState<roomImage[]>(roomImages);
  ...
  React.useEffect(() => {
    if (uploadedImages.length < 3) {
      setAlert({
        message:
          "Upload Images. Your Room is not enabled yet. Please upload some Images of you room here (Minimum 3 Images)",
        type: "info",
        show: true,
      });
    }
  ...
  const deleteHandler = async (id: number) => {
    ...
    await roomImagesService
      .deleteImage(id, roomId)
      .then((response) => {
        imageDeleteHandler(id);
        setUploadedImages((prev) => prev.filter((e) => e.id !== id) );
        setAlert({
  ...
  const handleCroppedImage = async (croppedImage: string) => {
    ...
    await fetch(croppedImage)
      ...
      .then(async (blob) => {
        const file = new File([blob], selectedImageName, {
          type: blob.type,
        });
        ...
        await roomImagesService
          ...
          .then(async (response) => {
            if (response.status === 200) {
              imageUploadHandler(response.data);
  ...
  return (
    <>
      <Modal
        openStatus={loading.status}
        closeHandler={null}
        ModalHeader=""
        closeButton={false}
      >
        <div
          className="inset-0 bg-gray-800 fixed flex w-full h-full opacity-50 items-center justify-center duration-300 transition-opacity"
          style={{ zIndex: 6000 }}
        >
          <div className="flex-col">
              ...
              <svg viewBox="0 0 860.1 876.5">
                <path
                  ...
                  style={{
                    stroke: "black",
                    strokeWidth: 1,
                    transformOrigin: "33.47285199% 32.89218482%",
                ...
                />
                <path
                  className="animate-spin from-yellow-300"
                  style={{
                    stroke: "black",
                    strokeWidth: 1,
                    transformOrigin: "78.02581095% 16.12093553%",
            ...
            <div className="mt-3 text-white font-mono text-sm sm:text-xs ">
              {loading.title}
            </div>
          </div>
        </div>
                ...
                <label className="border-dashed border-2 border-highlight py-12 flex flex-col justify-center items-center bg-gray-900 rounded-md appearance-none cursor-pointer hover:border-gray-400 focus:outline-none">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
                      You can add upto 5 Images. (Image will be cropped in 1:1
                      ratio)
                    </p>
                    <p className="text-xs text-red-800  text-center">
                      Minimum 3 Images is required*
                    </p>
                  </div>
                  ...
                  <input
                    ...
                    className="hidden"
                    disabled={loading.status || openImageModal}
                  />
                </label>
              </div>
            )}
                    ...
                    >
                      ...
                      >
                        <img
                          ...
                          src={image.resizedImageLink}
                        />

                        <section className="flex flex-col rounded-md text-xs break-words w-full h-full z-20 absolute top-0 py-2 px-3">
                          <h1 className="flex-1">Image&nbsp;{index + 1} </h1>
...
export default RoomImageDetails;
